...
For local browser selection, use `webcmd setup --mode local --browser cloak`, `webcmd setup --mode local --browser slab`, or `webcmd setup --mode local --browser /absolute/path/to/browser`. Cloak remains the bundled default; SLAB is the macOS alpha opt-in.
