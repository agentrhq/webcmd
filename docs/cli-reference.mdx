...
Local browser commands use the browser selected by `webcmd setup --mode local --browser ...`. Cloak stays bundled and default, `--browser slab` is the macOS alpha opt-in, and an absolute path selects a compatible local Chromium fork. Hosted browser commands use Webcmd Cloud and Browser Use; `web fetch` still runs locally.
...
`webcmd setup --status` returns the configured local `browser` selection in JSON. `webcmd doctor` reports the live `Runtime` plus a `Selected browser` line so you can tell whether local mode is using bundled Cloak, macOS-alpha SLAB, or a custom absolute executable path.
