...
cli({
    ...
    func: async (page) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('webcmd notebooklm current', 'No NotebookLM notebook is open in the adapter session. Run `webcmd notebooklm open <notebook>` first.');
        }
        const current = await readCurrentNotebooklm(page);
        if (!current) {
            throw new EmptyResultError('webcmd notebooklm current', 'NotebookLM notebook metadata was not found on the current page.');
        }
        return [current];
    },
});
