...
cli({
    ...
    func: async (page) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('webcmd notebooklm source-list', 'No NotebookLM notebook is open in the adapter session. Run `webcmd notebooklm open <notebook>` first.');
        }
        ...
        if (domRows.length > 0)
            return domRows;
        throw new EmptyResultError('webcmd notebooklm source-list', 'No NotebookLM sources were found on the current page.');
    },
});
