...
cli({
    ...
    func: async (page) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('webcmd notebooklm summary', 'No NotebookLM notebook is open in the adapter session. Run `webcmd notebooklm open <notebook>` first.');
        }
        ...
        if (rpcSummary)
            return [rpcSummary];
        throw new EmptyResultError('webcmd notebooklm summary', 'NotebookLM summary was not found on the current page.');
    },
});
