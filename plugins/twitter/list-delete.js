...
import { parseListsManagement } from './lists-parser.js';
...
cli({
    ...
    browser: true,
    args: [
        { name: 'listId', positional: true, type: 'string', required: true, help: 'Numeric ID of the list you own (e.g. from `webcmd twitter lists`)' },
    ...
    func: async (page, kwargs) => {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new ArgumentError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected numeric ID.`, 'Example: webcmd twitter list-delete 123456789 --confirm true');
        }
        if (!normalizeConfirm(kwargs.confirm)) {
            throw new ArgumentError('Refusing to delete list without --confirm true', 'Example: webcmd twitter list-delete 123456789 --confirm true');
        }
