...
// projection. Download stats are intentionally separate (see `pypi downloads`).
    ...
    func: async (args) => {
        ...
        for (const version of releaseVersions) {
            for (const file of releases[version]) {
                const time = String(file?.upload_time ?? '').slice(0, 10);
                if (!time) continue;
                if (!firstReleased || time < firstReleased) firstReleased = time;
                if (!lastReleased || time > lastReleased) lastReleased = time;
            }
        }
        return [{
