...
function resolveStateDb(args) {
    ...
    if (!fs.existsSync(db)) {
        throw new CommandExecutionError(
            ...
            'List valid workspace ids with `webcmd trae-solo workspaces-list`.',
        );
    }
    return db;
}
...
cli({
    ...
    func: async (args) => {
        ...
        if (val === null) {
            throw new CommandExecutionError(`Key not found: ${key}`, 'List available keys with `webcmd trae-solo state-keys`.');
        }
