...
export type LocalBrowserConfig =
  | { kind: 'cloak' }
  | { kind: 'chrome'; executablePath: string; chromeProfile?: string; syncToChrome?: boolean }
  | { kind: 'slab' }
...
function readLocalBrowser(value: unknown): LocalBrowserConfig {
  if (value && typeof value === 'object') {
    const browser = value as {
      kind?: unknown;
      executablePath?: unknown;
      chromeProfile?: unknown;
      syncToChrome?: unknown;
    };
    ...
    if (browser.kind === 'chrome' && typeof browser.executablePath === 'string' && path.isAbsolute(browser.executablePath)) {
      const chromeProfile = typeof browser.chromeProfile === 'string' && browser.chromeProfile.trim()
        ? browser.chromeProfile.trim()
        : undefined;
      return {
        kind: 'chrome',
        executablePath: browser.executablePath,
        ...(chromeProfile ? { chromeProfile } : {}),
        ...(browser.syncToChrome === true ? { syncToChrome: true } : {}),
      };
    }
