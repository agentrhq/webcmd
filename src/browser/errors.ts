...
export type BrowserErrorKind =
  | 'extension-transient'   // daemon/runtime hiccup — daemon-client retries
  ...
  switch (kind) {
    case 'daemon-not-running':
      return new BrowserConnectError(
        'Cannot connect to webcmd daemon.' + (detail ? `\n\n${detail}` : ''),
        `Run \`webcmd doctor\` to diagnose, or \`webcmd daemon restart\` to force a fresh daemon. Default port is ${DEFAULT_DAEMON_PORT}.`,
        kind,
      );
