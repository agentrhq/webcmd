...
- `vendor/client/network.ts` decodes response bodies and post data with the injected UTF-8 codec; `Buffer.prototype.toString('utf8')` is unavailable and `Uint8Array.prototype.toString` would comma-join the bytes. It also decodes binary WebSocket frame payloads with the injected base64 codec, since every page with a binary WebSocket delivers those frames whether or not the program listens for them.
