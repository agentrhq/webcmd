  ...
  if (isStale) {
    ...
    if (!portReleased) {
      throw new BrowserConnectError(
        ...
        '  Run manually: webcmd daemon stop',
        'daemon-not-running',
      );
    }
...
function browserConnectErrorFromHealth(health: DaemonHealth, contextId?: string): BrowserConnectError {
  if (health.state === 'profile-required') {
    return new BrowserConnectError(
      ...
      'Select one with --profile <name>, WEBCMD_PROFILE=<name>, or webcmd profile use <name>.\n' +
      'Run webcmd profile list to see connected profiles.',
      'profile-required',
    );
  }
