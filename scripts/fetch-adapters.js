...
/**
 * Sparse adapter sync: keeps ~/.webcmd/clis/ clean by removing stale overrides.
 *
 ...
 *
 * ~/.webcmd/clis/ is a sparse override layer, not a full copy.
 * Only eject-ed or user-modified sites appear here.
 *
 * Only runs on global install (npm install -g) or explicit WEBCMD_FETCH=1.
...
const WEBCMD_DIR = join(homedir(), '.webcmd');
const USER_CLIS_DIR = join(WEBCMD_DIR, 'clis');
const MANIFEST_PATH = join(WEBCMD_DIR, 'adapter-manifest.json');
const PACKAGE_ROOT = resolve(import.meta.dirname, '..');
const BUILTIN_CLIS = join(PACKAGE_ROOT, 'clis');

function log(msg) {
  console.log(`[webcmd] ${msg}`);
}
...
export function fetchAdapters() {
  ...
  // Skip if already installed at the same version (unless forced via WEBCMD_FETCH=1)
  const isForced = process.env.WEBCMD_FETCH === '1';
  ...
  //    Only eject-ed sites live in ~/.webcmd/clis/.
  let cleared = 0;
  ...
  // 4. Clean up legacy compat shim files from ~/.webcmd/
  // These were created by an older approach that placed re-export shims directly
  // in ~/.webcmd/ (e.g., registry.js, errors.js, browser/). The current approach
  // uses a node_modules/@agentrhq/webcmd symlink instead.
  ...
  let tmpCleaned = 0;
  try {
    for (const entry of readdirSync(WEBCMD_DIR)) {
      if (entry.startsWith('.plugins.lock.json.tmp-')) {
        try {
          unlinkSync(join(WEBCMD_DIR, entry));
          tmpCleaned++;
        } catch { /* ignore */ }
      }
    }
...
function main() {
  ...
  const isExplicit = process.env.WEBCMD_FETCH === '1';
  const isFirstRun = process.env._WEBCMD_FIRST_RUN === '1';
