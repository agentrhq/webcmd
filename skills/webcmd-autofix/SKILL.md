...
- **Hosted failure handoff:** if any failure returns `handoff.status === action_required`, this is a hard stop before trace collection or AutoFix. Show `handoff.viewUrl` to the user and wait for them to report done. Never request or enter credentials, passwords, or CAPTCHA answers. Then run `handoff.verifyCommand` when present. Without a `verifyCommand`, inspect fresh browser state and verify the intended post-action state before any retry, especially for write commands. Resume only after verification succeeds.
