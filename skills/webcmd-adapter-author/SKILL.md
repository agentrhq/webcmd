...
- After `webcmd plugin update`, check the reported overrides needing reconciliation and merge each using the three printed paths. `base` is the common ancestor, so this is a three-way merge, not a guess. Only overrides are reported; a user-authored adapter has no upstream.
