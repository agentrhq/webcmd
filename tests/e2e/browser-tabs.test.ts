...
// Match the running CLI's package version so the client does not classify
...
describe('browser tab CLI e2e', () => {
  ...
  it('keeps untargeted browser commands on the default tab after creating a new tab', async () => {
    const daemon = await startFakeDaemon();
    daemons.push(daemon);
    const cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'webcmd-browser-tabs-'));
    cacheDirs.push(cacheDir);
    const env = {
      WEBCMD_CACHE_DIR: cacheDir,
    };
