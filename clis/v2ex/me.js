...
cli({
    ...
    func: async (page) => {
        ...
        if (process.env.WEBCMD_VERBOSE) {
            console.error('[webcmd:v2ex] Navigating to /');
        }
    ...
    `);
        if (data.error) {
            if (process.env.WEBCMD_VERBOSE) {
                console.error(`[webcmd:v2ex:debug] Page Title: ${data.debug_title}`);
                console.error(`[webcmd:v2ex:debug] Page Body: ${data.debug_body}`);
            }
