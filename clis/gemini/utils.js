...
const GEMINI_COMPOSER_MARKER_ATTR = 'data-webcmd-gemini-composer';
...
function exportGeminiDeepResearchReportScript(maxWaitMs) {
    ...
    (async () => {
      ...
      const patchedKey = '__webcmdGeminiExportPatched';
      const trace = [];
      ...
      const ensureRecorder = () => {
        ...
        XMLHttpRequest.prototype.open = function(method, url, ...rest) {
          ...
          try { this.__webcmdReqUrl = String(url || ''); } catch {}
          ...
          try {
            this.addEventListener('load', () => {
              try {
                ...
                const reqUrl = String(this.__webcmdReqUrl || '');
                if (isDriveDocCreateRequest(reqUrl)) {
