...
describe('sanitizeGeminiResponseText', () => {
    it('strips a prompt echo only when it appears as a prefixed block', () => {
        const prompt = 'Reply with the word webcmd';
        const value = `Reply with the word webcmd\n\nwebcmd`;
        expect(sanitizeGeminiResponseText(value, prompt)).toBe('webcmd');
    });
    it('does not strip prompt text that appears later in a legitimate answer', () => {
        const prompt = 'webcmd';
        const value = 'You asked about webcmd, and webcmd is the right keyword here.';
