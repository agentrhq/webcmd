...
describe('validateAskModelValue', () => {
    ...
    it('mentions webcmd gemini models in error messages', () => {
        try { validateAskModelValue('pro'); } catch (e) {
            expect(e.message).toContain('webcmd gemini models');
        }
...
describe('gemini ask orchestration', () => {
    ...
    it('rejects aliases with error message mentioning canonical ids and webcmd gemini models', async () => {
        const page = createPageMock();
    ...
    it('includes available models and suggests webcmd gemini models in invalid-model error', async () => {
        const page = createPageMock();
        mocks.ensureGeminiPage.mockResolvedValue(undefined);
        setupDiscoveryWithoutThinking(page);
