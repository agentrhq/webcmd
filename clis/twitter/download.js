 ...
 *
 * Usage:
 *   webcmd twitter download elonmusk --limit 50 --output ./twitter
 *   webcmd twitter download --tweet-url https://x.com/xxx/status/123 --output ./twitter
 */
import { cli, Strategy } from '@agentrhq/webcmd/registry';
import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError, EmptyResultError } from '@agentrhq/webcmd/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        try {
            ...
            if (!username) {
                throw new ArgumentError('twitter download username must be a valid Twitter/X handle', 'Example: webcmd twitter download @jack --limit 20');
            }
