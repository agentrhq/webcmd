...
import { log } from '@agentrhq/webcmd/logger';
import { stripHtml } from './text.js';

function validatePositiveInt(value, name) {
  const n = Number(value);
  if (!Number.isInteger(n) || n <= 0) {
    throw new ArgumentError(`zhihu collection --${name} must be a positive integer`, 'Example: webcmd zhihu collection 83283292 --limit 20');
  }
  return n;
}
...
function validateNonNegativeInt(value, name) {
  const n = Number(value);
  if (!Number.isInteger(n) || n < 0) {
    throw new ArgumentError(`zhihu collection --${name} must be a non-negative integer`, 'Example: webcmd zhihu collection 83283292 --offset 0');
  }
  return n;
}
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    if (!/^\d+$/.test(collectionId)) {
      throw new ArgumentError('Collection ID must be numeric', 'Example: webcmd zhihu collection 83283292');
    }
