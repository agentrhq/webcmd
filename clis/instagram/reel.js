...
import { Page as BrowserPage } from '@agentrhq/webcmd/browser/page';
...
function buildSafeTempVideoPath(filePath) {
    ...
    return path.join(os.tmpdir(), `webcmd-instagram-video-real${ext}`);
}
    ...
    (() => {
      ...
      for (const root of roots) {
        ...
        for (const input of inputs) {
          ...
          input.setAttribute('data-webcmd-reel-upload-index', String(index));
          selectors.push('[data-webcmd-reel-upload-index="' + index + '"]');
          index += 1;
        }
      }
