...
async function findUploadSelectors(page, mediaItems) {
    ...
    ((includesVideo) => {
      ...
      document.querySelectorAll('[data-webcmd-ig-upload-index]').forEach((el) => el.removeAttribute('data-webcmd-ig-upload-index'));
      const selectors = ordered.map((input, index) => {
        input.setAttribute('data-webcmd-ig-upload-index', String(index));
        return '[data-webcmd-ig-upload-index="' + index + '"]';
      });
...
function extractSelectorIndex(selector) {
    const match = selector.match(/data-webcmd-ig-upload-index="(\d+)"/);
    if (!match)
        return null;
...
async function injectImageViaBrowser(page, imagePaths, selector) {
    ...
    const chunkKey = `__webcmdInstagramUpload_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const protocolCaptureEnabled = process.env.WEBCMD_INSTAGRAM_CAPTURE === '1';
