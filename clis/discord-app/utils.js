...
export function buildListChannelsScript() {
    return `
      (function __webcmdDiscordListChannels() {
...
export function buildListServersScript() {
    return `
      (function __webcmdDiscordListServers() {
...
export function buildReadMessagesScript(count) {
    ...
    return `
      (function __webcmdDiscordReadMessages(limit) {
...
export function buildListThreadsScript(limit) {
    ...
    return `
      (function __webcmdDiscordListThreads(limit) {
        function parseRoute(raw) {
          try {
            var url = new URL(raw, 'https://discord.com');
            var parts = url.pathname.split('/').filter(Boolean);
...
export function buildRouteStateScript() {
    return `
      (function __webcmdDiscordRouteState() {
        function parseRoute(raw) {
          try {
            var url = new URL(raw, 'https://discord.com');
            var parts = url.pathname.split('/').filter(Boolean);
...
export async function resolveDiscordChannelTarget(page, kwargs = {}, options = {}) {
    ...
    throw new ArgumentError(
        ...
        'Use "webcmd discord-app channels -f json" and retry with --url or numeric --guild/--channel ids.',
    );
}
