...
function normalizeEndpoint(value) {
  const endpoint = String(value || process.env.WEBCMD_CDP_ENDPOINT || 'http://127.0.0.1:39240').trim();
  if (!endpoint) {
    throw new ArgumentError('Set WEBCMD_CDP_ENDPOINT, for example http://127.0.0.1:39240');
  }
...
export const targetsCommand = cli({
  ...
  example: 'WEBCMD_CDP_ENDPOINT=http://127.0.0.1:39240 webcmd trae-cn targets -f table',
  strategy: Strategy.LOCAL,
  browser: false,
  args: [
    { name: 'endpoint', type: 'string', required: false, help: 'CDP endpoint (default: WEBCMD_CDP_ENDPOINT)' },
  ...
  func: async (kwargs) => {
    ...
    const wanted = process.env.WEBCMD_CDP_TARGET || '';
