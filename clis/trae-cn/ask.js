...
import { TimeoutError } from '@agentrhq/webcmd/errors';
import {
  activityScript,
  approveTraePrompts,
  isActivityComplete,
...
export const askCommand = cli({
  ...
  example: 'WEBCMD_CDP_ENDPOINT=http://127.0.0.1:39240 WEBCMD_CDP_TARGET=talk webcmd trae-cn ask "请只回复 OK" --timeout 120 -f json',
  domain: 'localhost',
    ...
    if (latest?.Text) {
      ...
      if (blocked) {
        throw new TimeoutError(
          'trae-cn ask',
          timeout,
          `Current status=${latestActivity?.Status ?? ''}, approvalPending=${latestActivity?.ApprovalPending ?? ''}, approvalKind=${latestActivity?.ApprovalKind ?? ''}, approvalButton=${latestActivity?.ApprovalButton ?? ''}. Use "webcmd trae-cn targets -f json" to find the waiting target, then run "WEBCMD_CDP_TARGET=<target> webcmd trae-cn watch --stream true --duration 300" or "webcmd trae-cn approve".`,
        );
      }
      return [latest];
    }
