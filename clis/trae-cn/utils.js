import { ArgumentError, CommandExecutionError, EmptyResultError, selectorError } from '@agentrhq/webcmd/errors';
  ...
  return `
    (function(kinds, click, limit, maxChars, markCandidates) {
      ...
      for (const candidate of candidates) {
        ...
        if (markCandidates) {
          const marker = 'webcmd-approval-' + rows.length + '-' + Math.random().toString(36).slice(2);
          candidate.button.setAttribute('data-webcmd-approval-id', marker);
          selector = '[data-webcmd-approval-id="' + marker + '"]';
        }
        const row = {
